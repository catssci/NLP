...
def scaled_dot_product_attention(query, key, value, mask):
  ...
  # 그래서 소프트맥스 함수를 지나기 전에 masking 해줌
  if mask is not None:
    logits += (mask * -1e9)
...
<ipython-input-9-fe93c72c5733> in <module>()
      1 a = 3
----> 2 assert a == 2
...
class MultiHeadAttention(tf.keras.layers.Layer):
  ...
  # 처음 shape : ~~~~********************************************** 체크 필요
  # 변환 shape : (batch_size, num_heads, query의 문장 길이, d_model/num_heads)
...
### 06. N 개의 Encoder_layer를 쌓아 만든 Encoder 구조
...
### 09. N 개의 Decoder_layer를 쌓아 만든 Decoder 구조
